import type { TemplateSection } from "@/domain/template-layout";

export const CERTIFICATE_OF_QUALITY_DEFAULT_SECTIONS: TemplateSection[] = [
  {
    id: "header_meta",
    label: "Header + Meta",
    description: "Certificate header, date and reference number.",
    x: 0,
    y: 0,
    w: 12,
    h: 8,
    minH: 8,
    cells: [
      { id: "qc_title", label: "CERTIFICATE OF QUALITY", x: 0, y: 0, w: 12, h: 2 },
      { id: "qc_date", label: "Date", x: 0, y: 2, w: 6, h: 2 },
      { id: "qc_ref", label: "Ref No", x: 6, y: 2, w: 6, h: 2 },
      { id: "qc_statement", label: "Statement", x: 0, y: 4, w: 12, h: 4 },
    ],
  },
  {
    id: "details",
    label: "Details",
    description: "Key certificate fields (mode, moisture, parties, goods, weights).",
    x: 0,
    y: 0,
    w: 12,
    h: 28,
    minH: 28,
    cells: [
      { id: "qc_mode", label: "Mode of Transportation", x: 0, y: 0, w: 6, h: 2 },
      { id: "qc_moisture", label: "Moisture Content", x: 6, y: 0, w: 6, h: 2 },
      { id: "qc_shipper", label: "Shipper", x: 0, y: 2, w: 12, h: 2 },
      { id: "qc_notify", label: "Notify", x: 0, y: 4, w: 12, h: 2 },
      { id: "qc_notify2", label: "Second Notify", x: 0, y: 6, w: 12, h: 2 },
      { id: "qc_desc", label: "Description of Goods", x: 0, y: 8, w: 12, h: 2 },
      { id: "qc_origin", label: "Origin", x: 0, y: 10, w: 12, h: 2 },
      { id: "qc_quality", label: "Quality", x: 0, y: 12, w: 12, h: 2 },
      { id: "qc_ico_no", label: "ICO No", x: 0, y: 14, w: 12, h: 2 },
      { id: "qc_cert_no", label: "Cert No", x: 0, y: 16, w: 12, h: 2 },
      { id: "qc_net_weight", label: "Net Weight", x: 0, y: 18, w: 12, h: 2 },
      { id: "qc_gross_weight", label: "Gross Weight", x: 0, y: 20, w: 12, h: 2 },
      { id: "qc_qty_lb", label: "Quantity in LB", x: 0, y: 22, w: 12, h: 2 },
      { id: "qc_from", label: "From", x: 0, y: 24, w: 12, h: 2 },
      { id: "qc_to", label: "To", x: 0, y: 26, w: 12, h: 2 },
    ],
  },
  {
    id: "container_table",
    label: "Containers Table",
    description: "Container No / Seal No / Bags per container list.",
    x: 0,
    y: 0,
    w: 12,
    h: 6,
    minH: 6,
    cells: [
      { id: "qc_ct_container", label: "Container No", x: 0, y: 0, w: 4, h: 2 },
      { id: "qc_ct_seal", label: "Seal No", x: 4, y: 0, w: 4, h: 2 },
      { id: "qc_ct_bags", label: "Quantity of Bags per Container", x: 8, y: 0, w: 4, h: 2 },
      { id: "qc_ct_row", label: "Row values…", x: 0, y: 2, w: 12, h: 4 },
    ],
  },
  {
    id: "footer",
    label: "Footer",
    description: "Signatory company line.",
    x: 0,
    y: 0,
    w: 12,
    h: 4,
    minH: 4,
    cells: [
      { id: "qc_signatory", label: "Signatory Company", x: 0, y: 0, w: 12, h: 2 },
    ],
  },
  {
    id: "document_id",
    label: "Document ID",
    description: "Footer document ID line.",
    x: 0,
    y: 0,
    w: 12,
    h: 4,
    minH: 4,
    showDocumentId: false,
    cells: [],
  },
];
